...
def get_date_interval(sentence):
    if "tomorrow" in sentence:
            ...
            #if int(datetime.date.today().strftime("%d")) != 31:
...
def check_product(name_list, sentence, p, g, d, s):
    ...
    for theatre in showtime_theatres:
        for word in theatre[0]:
            if not word.isdigit() and word in sentence:
                count += 1
    ...
    if filtered == []:
        ...
        for product in filtered:
            connector.query("INSERT INTO bi_response (response_id,tweet_id,user_id,product_id,created_at) VALUES (" + str(res_id) + ",0,0," + str(product[3]) + ",0)")
        return res_id
...
print(response)
#print(movie("comedy for adults starring channing tatum directed by phil lord playing from 7 to 8:45 at carlton theatres"))